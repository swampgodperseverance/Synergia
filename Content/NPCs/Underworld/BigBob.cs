...
namespace Synergia.Content.NPCs.Underworld {
    public class BigBob : ModNPC {
        ...
        int attackCooldown;
        float flashTimer;
        bool hasFiredThisCast;
        Vector2 retreatVelocity;
        Player closestPlayer;

        const int PRECAST_DURATION = 96;
        const int CAST_DURATION = 84;
        const int POSTCAST_DURATION = 54;
        const int MIN_ATTACK_GAP = 270;
        const int MAX_ATTACK_GAP = 390;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor) {
            if (flashTimer > 0f) {
                ...
                Texture2D glow = Request<Texture2D>(GetTexturesElementName("LightTrail_1")).Value;
                Vector2 origin = glow.Size() / 2f;

                Main.EntitySpriteDraw(glow, NPC.Center - Main.screenPosition, null, flashColor * 0.8f, 0f, origin, NPC.scale * 1.2f * intensity, SpriteEffects.None, 0);
                Main.EntitySpriteDraw(glow, NPC.Center - Main.screenPosition, null, flashColor * 0.3f, 0f, origin, NPC.scale * 1.8f * intensity, SpriteEffects.None, 0);
