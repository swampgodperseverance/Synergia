        ...
        public override void AI()
        {
            ...
            if (heartbeatSoundTimer <= 0)
            {
                if (screenFade > 0.3f && !dying)
                {
                    ...
                    if (lifeRatio < 0.3f && cameraShakeTimer <= 0)
                    {
                        var shakePlayer = Main.LocalPlayer.GetModPlayer<ScreenShakePlayer>();
                        shakePlayer?.TriggerShake(12, 0.8f);
                        cameraShakeTimer = 0.4f;
                    }
                }
            }
            else
            {
                heartbeatSoundTimer -= 0.016f;
            }
            ...
            if (lifeRatio <= 0.5f && !dying)
            {
                ...
                shakeIntensity = shakeFactor * 1.5f * (0.8f + heartbeatPulse * 0.4f);
