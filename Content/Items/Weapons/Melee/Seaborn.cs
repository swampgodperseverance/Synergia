...
namespace Synergia.Content.Items.Weapons.Melee
{
    public class Seaborn : ModItem
    {
        ...
        public override void HoldItem(Player player)
        {
            ...
            // ПКМ активация SeabornAttack2
            if (Main.mouseRight && specialCooldown == 0)
            {
                specialCooldown = 600; // 10 секунд

                // Спавним SeabornAttack2 над курсором
                Vector2 spawnPosition = Main.MouseWorld;      // позиция курсора в мире
                spawnPosition.Y -= 40f;                      // поднимем на 100 пикселей над курсором (можно менять)

                Projectile.NewProjectile(
                    player.GetSource_ItemUse(Item),
                    spawnPosition,
                    Vector2.Zero,                             // без начальной скорости
                    ModContent.ProjectileType<SeabornAttack2>(),
                    Item.damage,
                    Item.knockBack,
                    player.whoAmI
                );

                SoundEngine.PlaySound(SoundID.Item29, spawnPosition); // звук
            }
        ...
        if (Main.rand.NextFloat() < 0.12f)
        {
            ...
            dust = Main.dust[
                Terraria.Dust.NewDust(position, Item.width, Item.height, DustID.DungeonWater, 0f, -10f, 0,
                    new Color(150, 200, 255), 1f)];
        }
        ...
        class SeabornSlash : ModProjectile
        {
            public static int empoweredHitsLeft = 0;    
            public override void SetStaticDefaults()
            {
            ...
            public override void OnHitNPC(NPC npc, NPC.HitInfo hit, int damageDone)
            {
                ...
                if (hitcount >= 4)
                {
                    ...
                    Projectile.NewProjectile(Projectile.InheritSource(Projectile), pos + new Vector2(10, Main.rand.NextFloat(0, -60)), new Vector2(0, 18), ModContent.ProjectileType<SeabornSword>(), Projectile.damage * 2, Projectile.knockBack, player.whoAmI);
                    Projectile.NewProjectile(Projectile.InheritSource(Projectile), pos + new Vector2(-10, Main.rand.NextFloat(0, -60)), new Vector2(0, 18), ModContent.ProjectileType<SeabornSword>(), Projectile.damage * 2, Projectile.knockBack, player.whoAmI);
                   ...
                   for (int i = 0; i < 20; i++)
                {
                    int dust = Dust.NewDust(pos, Projectile.width, Projectile.height, DustID.DungeonWater, Projectile.oldVelocity.X * 0.2f, Projectile.oldVelocity.Y * 0.2f);
    ...
    public class SeabornSword : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int k = 0; k < 10; k++)
            {
                int dust = Dust.NewDust(Projectile.position + Projectile.velocity, Projectile.width, Projectile.height, DustID.DungeonWater, Projectile.oldVelocity.X * 0.2f, Projectile.oldVelocity.Y * 0.2f);
