...
public class ScorcherHeldItemProj : ModProjectile
{
	...
	private void SpawnLaser(Vector2 center, Player player)
	{
        ...
        Vector2 newPos = center + Projectile.velocity + new Vector2(Projectile.width / 2, -7 * Projectile.direction).RotatedBy(velocity.ToRotation());
		Vector2 beamStartPos = newPos;
		Projectile.NewProjectile(Projectile.GetSource_FromAI(), newPos, velocity, type, Projectile.damage, Projectile.knockBack, player.whoAmI);
