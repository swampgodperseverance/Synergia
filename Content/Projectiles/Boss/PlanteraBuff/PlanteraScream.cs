...
{
    public class PlanteraScream : ModProjectile
    {
        ...
        {
            if (Projectile.ai[1] == 2f && Projectile.timeLeft == 29 && Main.myPlayer == Projectile.owner)
                Main.player[Projectile.owner].GetModPlayer<ScreenShakePlayer>().TriggerShake(6, 0.8f);
        }

        public override bool PreDraw(ref Color lightColor)
        {
            lightColor = (Projectile.ai[1] == 1f ? Color.HotPink : Color.Lime) * ((float)Projectile.timeLeft / 90f);
            lightColor.A = 0;
