...
{
	public class JadeExplosion : ModProjectile
	{
		...
		public override void AI() {
			...
			if(Projectile.localAI[0] == 0) {
				...
				for(int i = 0; i < 3; i++) {
					int l = Dust.NewDust(Projectile.Center + Main.rand.NextVector2Circular(15, 15), 0, 0, ModContent.DustType<JadeDust>());
					Main.dust[l].alpha = 200;
				...
				if(Projectile.timeLeft < 3) color = Color.Lerp(Color.Black, Color.Turquoise, (float)Projectile.timeLeft / 3f);
				else  color = Color.Lerp(Color.Turquoise, Color.Teal, (float)System.Math.Sqrt((float)(Projectile.timeLeft - 3) / 3f));
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			if(Projectile.ai[0] > 0f) {
				...
				Main.EntitySpriteDraw(texture, Projectile.Center - Main.screenPosition, null, Color.Turquoise with {A = 0} * 0.1f * Projectile.Opacity, Main.rand.NextFloat(MathHelper.TwoPi), texture.Size() / 2, 0.6f * Projectile.scale, SpriteEffects.None, 0);
				return false;
			}
			if(Projectile.timeLeft < 3) lightColor = Color.Lerp(Color.Black, Color.Turquoise, (float)Projectile.timeLeft / 3f);
			else lightColor = Color.Lerp(Color.Turquoise, Color.Teal, (float)System.Math.Sqrt((float)(Projectile.timeLeft - 3) / 6f));
