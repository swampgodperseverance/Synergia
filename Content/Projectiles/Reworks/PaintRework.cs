...
{
    public class PaintRework : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int k = 0; k < Projectile.oldPos.Length - 1; k++)
            {
                ...
                float hue = (Main.GlobalTimeWrappedHourly * 0.5f + k * 0.1f) % 1f;
                Color rainbowColor = Main.hslToRgb(hue, 1f, 0.5f) * ((120 - k * 8) / 255f);

                spriteBatch.Draw(glow, Projectile.oldPos[k] + glowPosition, null, rainbowColor, glowRot, glowOrigin, Projectile.scale - k / (float)Projectile.oldPos.Length, spriteEffects, 0f);
                spriteBatch.Draw(glow, (Projectile.oldPos[k] + Projectile.oldPos[k + 1]) * 0.5f + glowPosition, null, rainbowColor, glowRot, glowOrigin, Projectile.scale - k / (float)Projectile.oldPos.Length, spriteEffects, 0f);
            }
