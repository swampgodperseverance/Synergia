...
{
    public class MarbleRework : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (!player.frozen)
            {
                ...
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    {
                        // Пыль мрамора
                        Dust dust = Dust.NewDustDirect(Projectile.Center, 14, 14, DustID.Marble, 0f, 0f, 120);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            // Наложение BrokenShield
            target.AddBuff(ModContent.BuffType<ValhallaMod.Buffs.EnemyOnly.BrokenShield>(), 300);
        }
              ...
              public override bool PreDraw(ref Color lightColor)
        {
            ...
            // Загружаем текстуру мраморного копья
            Texture2D texture = (Texture2D)ModContent.Request<Texture2D>(Texture).Value;
            ...
            for (int k = 0; k < Projectile.oldPos.Length - 1; k++)
            {
                ...
                Color glowColor = new Color(200 - k * 20, 200 - k * 20, 200 - k * 20, 20);
                spriteBatch.Draw(glow, Projectile.oldPos[k] + glowPosition, null, glowColor, glowRot, glowOrigin,
                                 Projectile.scale - k / (float)Projectile.oldPos.Length, spriteEffects, 0f);
                spriteBatch.Draw(glow, (Projectile.oldPos[k] + Projectile.oldPos[k + 1]) * 0.5f + glowPosition, null,
                                 glowColor, glowRot, glowOrigin, Projectile.scale - k / (float)Projectile.oldPos.Length,
                                 spriteEffects, 0f);
            }
