...
{
    public class ChlorophyteRework : ModProjectile
    {
        ...
        public override void AI() {
            ...
            if (Projectile.ai[0] == 1) {
                ...
                // 🌱 Shoot a Chlorophyte SporeCloud on swing
                if (Main.myPlayer == Projectile.owner) {
                    Vector2 shootVel = Projectile.velocity.SafeNormalize(Vector2.UnitY) * 8f;
                    Projectile.NewProjectile(
                        Projectile.GetSource_FromThis(),
                        Projectile.Center,
                        shootVel.RotatedByRandom(0.5f), // some spread
                        ProjectileID.SporeCloud, // Synergia spore projectile
            ...
            if (!player.frozen) {
                ...
                if (Main.netMode != NetmodeID.Server) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        Dust dust = Dust.NewDustDirect(Projectile.Center, 14, 14, DustID.Grass, 0f, 0f, 120);
                        dust.color = new Color(50, 255, 80); // bright green
                        dust.velocity = origin.DirectionTo(dust.position) * 2f;
                        dust.position = Projectile.Center + normVel.RotatedBy(
                            (1f - animProgress) * ((float)Math.PI * 2f) * 2f + i / 2f * ((float)Math.PI * 2f)
                        ) * 8f;
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            for (int k = 0; k < Projectile.oldPos.Length - 1; k++) {
                ...
                // 🌱 Chlorophyte green glow
                Color glowColor = new Color(80, 255, 120, 150);
