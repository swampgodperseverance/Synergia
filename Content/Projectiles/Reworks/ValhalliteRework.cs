...
{
    public class ValhalliteRework : ModProjectile
    {
        private float glowRotation;
        private const float maxReach = 90f;
        private const float minReach = 10f;
        private int soulTimer = 0; // Таймер для выпуска ValhalliteSoul
        ...
        public override void AI()
        {
            ...
            if (!player.frozen)
            {
                ...
                Vector2 spinningpoint = new Vector2(1.5f, 0f)
                    .RotatedBy(Math.PI + (1f - animProgress) * (Math.PI * 3f)) * new Vector2(velLen, Projectile.ai[0]);

                Projectile.position += spinningpoint.RotatedBy(velRot) + new Vector2(velLen + reach, 0f).RotatedBy(velRot);
                Vector2 target = origin + spinningpoint.RotatedBy(velRot) + new Vector2(velLen + reach, 0f).RotatedBy(velRot);
                ...
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        // Модовая пыль Valhallite
                        Dust dust = Dust.NewDustDirect(Projectile.Center, 14, 14, ModContent.DustType<ValhallaMod.Dusts.ValhalliteDust>(), 0f, 0f, 120);
                        dust.velocity = origin.DirectionTo(dust.position) * 2f;
                        dust.position = Projectile.Center + normVel.RotatedBy((1f - animProgress) * ((float)Math.PI * 2f) * 2f + i / 2f * ((float)Math.PI * 2f)) * 8f;
                        dust.scale = Main.rand.NextFloat(0.8f, 1.4f);
                        dust.velocity += normVel * 3f;
                        dust.noGravity = true;
                        dust.noLight = false;
                        dust.color = new Color(180, 220, 255, 200);
                    }
                }
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            ...
            Vector2 spinningpoint = new Vector2(1.5f, 0f)
                .RotatedBy(Math.PI + (1f - animProgress) * (Math.PI * 3f)) * new Vector2(velLen, Projectile.ai[0]);
            Vector2 spearTip = origin + spinningpoint.RotatedBy(velRot) + new Vector2(velLen + reach, 0f).RotatedBy(velRot);

            float collisionPoint = 0f;
            if (Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), origin, spearTip, 23f * Projectile.scale, ref collisionPoint))
                return true;
            return false;
        }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (soulTimer >= 10)
            {
                ...
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, Vector2.Zero, ModContent.ProjectileType<ValhalliteSoul>(), Projectile.damage / 2, 1f, Projectile.owner);
                }
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int k = 0; k < Projectile.oldPos.Length - 1; k++)
            {
                ...
                float glowRot = (float)Math.Atan2(Projectile.oldPos[k].Y - Projectile.oldPos[k + 1].Y, Projectile.oldPos[k].X - Projectile.oldPos[k + 1].X);
                Color glowColor = new Color(180, 220, 255, 120 - k * 8);
