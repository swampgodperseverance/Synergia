...
{
    public class NaquadahRework : ModProjectile
    {
        private float glowRotation;
        private const float maxReach = 200f; // Вернули для визуала
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox) {
            ...
            // Рассчитаем фактическое смещение конца копья, как в AI()
            Vector2 spinningpoint = new Vector2(1.5f, 0f)
                .RotatedBy(Math.PI + (1f - animProgress) * (Math.PI * 3f))
                * new Vector2(velLen, Projectile.ai[0]);
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            for (int k = 0; k < Projectile.oldPos.Length - 1; k++) {
                ...
                Color glowColor = new Color(160 - k * 30, 50, 78 + k * 30, 20);
