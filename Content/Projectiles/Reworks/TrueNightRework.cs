...
{
    public class TrueNightRework : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (!player.frozen)
            {
                ...
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        Dust dust = Dust.NewDustDirect(Projectile.Center, 14, 14, ModContent.DustType<DurataniumDust>(), 0f, 0f, 120);
                        dust.color = new Color(75, 0, 130); // тёмно-фиолетовый
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            ShootExtraProjectiles(target.Center);
        }
        ...
        private void ShootExtraProjectiles(Vector2 targetPos)
        {
            if (Main.myPlayer == Projectile.owner)
            {
                for (int i = 0; i < 3; i++)
                {
                    Vector2 velocity = Vector2.UnitX.RotatedByRandom(MathHelper.ToRadians(45)) * 10f;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int k = 0; k < Projectile.oldPos.Length - 1; k++)
            {
                ...
                Color glowColor = new Color(138 - k * 10, 43, 226 - k * 20, 20);
