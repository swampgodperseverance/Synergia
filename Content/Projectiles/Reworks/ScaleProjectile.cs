...
{
    public class ScaleProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // Эффект взрыва при исчезновении
                    if (!bloodExplosionDone && Projectile.alpha >= 128)
                    {
                        ...
                        for (int i = 0; i < 30; i++)
                        {
                            ...
                            Dust dust = Dust.NewDustDirect(Projectile.Center, 0, 0, DustID.CorruptGibs, dustVel.X, dustVel.Y, 100, default, Main.rand.NextFloat(0.8f, 1.5f));
                            dust.noGravity = true;
                        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            // Отрисовка трейла (изменен цвет на пурпурный)
            for (int k = 0; k < Projectile.oldPos.Length; k++)
            {
                ...
                Color color = new Color(180, 0, 180, 150) * scale * (1f - Projectile.alpha / 255f); // Пурпурный цвет
                spriteBatch.Draw(texture, drawPos, null, color, Projectile.rotation, origin, scale, SpriteEffects.None, 0f);
            }

            // Отрисовка основного спрайта
            Color drawColor = Color.Purple * (1f - Projectile.alpha / 255f);
