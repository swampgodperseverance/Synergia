...
{
    public class TerraRework : ModProjectile
    {
        private float glowRotation;
        private const float maxReach = 300f; 
        ...
        public override void AI() {
            ...
            if (!player.frozen) {
                ...
                if (Main.netMode != NetmodeID.Server) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        Dust dust = Dust.NewDustDirect(Projectile.Center, 14, 14, DustID.GreenTorch, 0f, 0f, 120);
            ...
            if (player.itemAnimation == 2) {
                if (Main.myPlayer == player.whoAmI) {
                    for (int i = -1; i <= 1; i++) {
                        Vector2 shootVelocity = Projectile.velocity.RotatedBy(MathHelper.ToRadians(10 * i));
                        Projectile.NewProjectile(
                            Projectile.GetSource_FromThis(),
                            Projectile.Center,
                            shootVelocity,
                            ModContent.ProjectileType<TerraTridientProjectile>(),
                            Projectile.damage,
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            for (int k = 0; k < Projectile.oldPos.Length - 1; k++) {
                ...
                Color glowColor = new Color(0, 255, 0, 100 - k * 15); 
