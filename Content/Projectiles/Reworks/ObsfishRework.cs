...
{
    public class ObsfishRework : ModProjectile
    {
        private float glowRotation;
        private const float maxReach = 60f; 
        ...
        public override void AI() {
            ...
            if (!player.frozen) {
                ...
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        Dust dust = Dust.NewDustDirect(Projectile.Center, 14, 14, DustID.Torch, 0f, 0f, 120);
                        ...
                        dust.noGravity = true;
                        dust.noLight = false;
                        dust.color = new Color(255, 100, 0, 180); 
                    }

                    if (Main.rand.NextBool(5))
                    {
                        Vector2 bubbleVelocity = new Vector2(Main.rand.NextFloat(-1f, 1f), Main.rand.NextFloat(-2f, -0.5f));

                        int bubbleProj = 0;

                        if (ModLoader.TryGetMod("RoA", out Mod riseOfAges))
                        {
                            // Получаем тип проектайла Hellbat из RoA
                            int hellbatType = riseOfAges.Find<ModProjectile>("Hellbat")?.Type ?? 0;
                            if (hellbatType != 0)
                            {
                                bubbleProj = Projectile.NewProjectile(
                        ...
                        if (bubbleProj > 0)
                        {
                            Main.projectile[bubbleProj].timeLeft = 120;
                            Main.projectile[bubbleProj].GetAlpha(new Color(255, 150, 0));
                        }
                    }
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            for (int k = 0; k < Projectile.oldPos.Length - 1; k++) {
                ...
                Color glowColor = new Color(255, 100 + k * 7, 0, 120 - k * 8); 
