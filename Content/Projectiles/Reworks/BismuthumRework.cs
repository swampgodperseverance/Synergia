    ...
    public class BismuthumRework : ModProjectile {
        ...
        public override void AI() {
            ...
            if (Main.rand.NextFloat() < Projectile.Opacity) {
                Dust dust = Dust.NewDustPerfect(Projectile.Center + offset.ToRotationVector2() * (Main.rand.NextFloat() * 40f * Projectile.scale + 10f * Projectile.scale), 27, velocity * 1f, 50, new Color(180, 180, 200), 0.4f);
                dust.fadeIn = 0.3f + Main.rand.NextFloat() * 0.15f;
                dust.noGravity = true;
            }

            if (Main.rand.NextFloat() * 1.5f < Projectile.Opacity) {
                Dust dust2 = Dust.NewDustPerfect(position, 27, velocity * 1.5f, 100, new Color(220, 220, 240) * Projectile.Opacity, Projectile.Opacity);
        ...
        private void DrawLikeExcalibur(SpriteBatch spriteBatch) {
            ...
            Color value = Color.Lerp(new Color(100, 100, 120, 220), new Color(180, 180, 200, 220), lerpTime); 
            ...
            Color value2 = Color.Lerp(new Color(120, 120, 140, 220), new Color(200, 200, 220, 220), lerpTime) * 1.25f; 
            Color color = Color.Lerp(new Color(100, 100, 120, 220), new Color(220, 220, 240, 220), lerpTime) * 1.25f; 
            ...
            for (float i = 0f; i < 12f; i += 1f) {
                ...
                float scale = i / 12f;
                DrawHelper.DrawPrettyStarSparkle(Projectile.Opacity, SpriteEffects.None, drawPos, Color.Lerp(new Color(230, 230, 250, 0), new Color(200, 200, 230, 0), percentageOfLife) * lerpTime * 2f * scale, color, percentageOfLife, 0f, 0.5f, 0.5f, 1f, edgeRotation, new Vector2(0f, Utils.Remap(percentageOfLife, 0f, 1f, 3f, 0f)) * scaleFactor, Vector2.One * scaleFactor);
            }
            ...
            DrawHelper.DrawPrettyStarSparkle(Projectile.Opacity, SpriteEffects.None, drawpos2, Color.Lerp(new Color(230, 230, 250, 0), new Color(200, 200, 230, 0), percentageOfLife) * lerpTime * 1.5f, color, percentageOfLife, 0f, 0.5f, 0.5f, 1f, 0f, new Vector2(2f, Utils.Remap(percentageOfLife, 0f, 1f, 4f, 1f)) * scaleFactor, Vector2.One * scaleFactor);
        }
