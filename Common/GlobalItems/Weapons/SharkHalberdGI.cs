...
{
    public class SharkHalberdGI : GlobalItem
    {
        ...
        public override bool AppliesToEntity(Item entity, bool lateInstantiation)
        {
            ...
            return string.Equals(modName, "Bismuth", System.StringComparison.OrdinalIgnoreCase)
                && string.Equals(itemName, "SharkHalberd", System.StringComparison.OrdinalIgnoreCase);
        }
        ...
        public override void SetDefaults(Item item)
        {
            ...
            item.shoot = ModContent.ProjectileType<SharkHalberdRework>();
            item.channel = true;
        ...
        {
            return player.ownedProjectileCounts[ModContent.ProjectileType<SharkHalberdRework>()] < 1;
        }
        ...
        {
            int idx = Projectile.NewProjectile(source, position, velocity, ModContent.ProjectileType<SharkHalberdRework>(), damage, knockback, player.whoAmI);
        ...
        {
            var mp = player.GetModPlayer<SharkPlayer>();
            if (mp != null && mp.SharkEmpowered)
                item.damage = 80;
            else
                item.damage = 35;
    ...
    public class SharkPlayer : ModPlayer
    {

        public int SharkHitCount = 0;
        public bool SharkEmpowered = false;
        ...
        {
            tag["SharkHitCount"] = SharkHitCount;
            tag["SharkEmpowered"] = SharkEmpowered;
        }

        public override void LoadData(TagCompound tag)
        {
            if (tag.ContainsKey("SharkHitCount")) SharkHitCount = tag.GetInt("SharkHitCount");
            if (tag.ContainsKey("SharkEmpowered")) SharkEmpowered = tag.GetBool("SharkEmpowered");
        }
    }
}
