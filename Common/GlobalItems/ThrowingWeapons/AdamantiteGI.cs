...
{
    public class ADAMANTITEGlobalItem : ThrowingGI {
        public override string AbilityInfo => "AdamantDaggerInfo";
        public override int ItemType => ModContent.ItemType<AdamantiteDagger>(); 
        public override bool NewBehavior(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
            int newType = ModContent.ProjectileType<AdamantiteDagger2>();
            velocity *= 1.2f;
