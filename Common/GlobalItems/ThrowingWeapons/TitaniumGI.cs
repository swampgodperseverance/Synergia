...
{
    public class tITANIUMglobalItem : ThrowingGI {
        public override string AbilityInfo => "TitaniumWarhammerInfo";
        public override int ItemType => ModContent.ItemType<TitaniumWarhammer>(); 
        public override bool NewBehavior(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
            int newType = ModContent.ProjectileType<TitaniumWarhammer2>();
            velocity *= 1.2f;
