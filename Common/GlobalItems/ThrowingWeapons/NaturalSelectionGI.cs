...
{
    public class NAturalSelectionGlobalItem : ThrowingGI {

        public override string AbilityInfo => "NaturalSelectionInfo";
        public override int ItemType => ModContent.ItemType<NaturalSelection>(); 
        public override bool NewBehavior(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
            int newType = ModContent.ProjectileType <NaturalSelection2> ();
            velocity *= 1.1f;
