...
{
    public class ChlorophytePiercerNHGlobalItem : ThrowingGI {
        public override string AbilityInfo => "ChlorophytePiercerInfo";
        public override int ItemType => ModContent.ItemType<ChlorophyteJavelin>(); 
        public override bool NewBehavior(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
            int newType = ModContent.ProjectileType<ChlorophytePiercer2>();
            velocity *= 1.5f;
