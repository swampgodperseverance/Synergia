...
{
    public class OrKaGlobalItem : ThrowingGI {
        public override string AbilityInfo => "OrKaInfo";
        public override int ItemType => ModContent.ItemType<OrichalcumKama>(); 
        public override bool NewBehavior(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
            int newType = ModContent.ProjectileType<OrichalcumKama2>();
            velocity *= 1.1f;
