...
namespace Synergia.Common.ModSystems.ModSupports {
    public class BossChecklistSupports : ModSupportsSystem {
        ...
        public override void PostSetupContent(Mod bossChecklistMod) {
            ...
            RegisterInBossChecklist(bossChecklistMod, type[0], nameof(Sinlord), 13.9f, () => DownedBossSystem.DownedSinlordBoss, NPCType<Sinlord>(), ItemType<HellwormScale>(), CustomPortrait("Sinlord", 1f));
            RegisterMiniBoss(
                bossChecklistMod,
                nameof(Cruor),
                12.5f,
                () => DownedBossSystem.DownedCruorBoss,
                NPCType<Cruor>(),
                CustomPortrait("Cruor", 1f)
            );
        }
        ...
        void RegisterMiniBoss(
        Mod bossChecklistMod,
        string internalName,
        float weight,
        Func<bool> downed,
        int bossType,
        Action<SpriteBatch, Rectangle, Color> customPortrait)
            {
                bossChecklistMod.Call(
                    ...
                    ["spawnInfo"] = "Appears in the Dungeon after collecting 3 Soulful Gems"
                );
            }
