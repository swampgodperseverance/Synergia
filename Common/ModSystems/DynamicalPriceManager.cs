...
//using Synergia.Core.PriceSystem;
//using Terraria;
...
//            // Цветовая схема (оставил как было)
//            float hue = mult < 0.65f ? 0f : mult < 0.85f ? 0.1f : 0.3f;
//            float targetR, targetG, targetB;
//            if (hue < 0.05f)
//            {
//                targetR = 1f; targetG = 0.3f + (1f - colorTransition) * 0.3f; targetB = 0.3f + (1f - colorTransition) * 0.3f;
//            }
//            else if (hue < 0.2f)
//            {
//                targetR = 1f; targetG = 0.6f + (1f - colorTransition) * 0.2f; targetB = 0.2f + (1f - colorTransition) * 0.3f;
//            }
//            else
//            {
...
//            Rectangle rect = new Rectangle((int)pos.X - 12, (int)pos.Y - 32, 220, 65);
//            sb.Draw(bgTex, rect, bgColor);

//            float borderAlpha = 0.6f * opacity;
//            DrawBorder(sb, borderTex, rect, Color.Goldenrod * borderAlpha);

//            // Только текущая цена (без Base)
//            Utils.DrawBorderStringFourWay(sb, FontAssets.MouseText.Value,
//                $"{DynamicPriceManager.GetValue()}",
//                pos.X, pos.Y - 18, textColor, Color.Black * opacity, Vector2.Zero, 0.85f);

//            // Процент напротив цены
